...
export default {
	...
	methods: {
		...
		updateNewTag(ievent) {
			console.log("●updateNewTag")
			const value = ievent.target.value;
			this.newTag = value;
			this.$emit('input', value);
		},
		initTags() {
			console.log("● initTags:", this.value);
			this.tagsCopy = this.value.map(id => {
	...
	watch: {
		value(newValue){
			console.log("●watch value1", newValue);
